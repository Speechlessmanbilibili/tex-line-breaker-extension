...
- 行首/行末悬挂标点/光学边缘对齐；全角句末标点采用压缩+悬挂，并仅在该行已经自然接近行末、无需明显拉伸时给予断行偏好，避免把中途结束的短句强行拉到行末。排版期间会对实际阻挡悬挂标点的祖先裁剪容器应用高优先级 `overflow: visible`，关闭或恢复原生布局时还原原值。
...
- 生成的逐行块固定使用起始对齐，避免继承站点的 `text-align: justify` 后被浏览器再次两端对齐；段末和强制断行的短行保持自然长度。

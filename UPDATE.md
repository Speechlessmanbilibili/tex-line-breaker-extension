...
- 修复逐行 `span` 继承 `text-align: justify` 后被浏览器进行第二次两端对齐的问题。
- 段落末行、强制断行和 iframe 内短行不再被均匀拉伸至整行。
- 行内松紧只采用 KP 核心计算出的 `letter-spacing`，不再叠加浏览器原生 justify。

# v0.3.3

- 将悬挂标点从强制性断行奖励改为条件性平局偏好。
- 只有候选行已经自然接近行末且字间拉伸较小时才奖励标点悬挂。
- 中途结束的短句不再为了把句末标点送到行末而被强行拉伸。
